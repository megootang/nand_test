...
//#define WRITE2NAND	//if defined，print all log to mem_test.txt
#ifdef	WRITE2NAND
...
//only print emmc err log
#define LOCAL_LOG_ERR(format,args...) do{\
			struct timeval now_time = {0};\
			gettimeofday(&now_time,(struct timezone*)0);\
			memset(gbuffer,0,BUFFER_LEN);\
			sprintf(gbuffer,"fuction: %s, line: %d, "format", %lu(us), %s", \
...
while(loop){
	...
	if(cyc_num) {
		...
		for(l = 0; l < cyc_num ; l++ ){ //write filesize / BUFF_SIZE BUFF_SIZE data
		    ...
		    if (write(fd, buf, size) != size) {
                write_flag ++;
		        LOCAL_LOG_ERR(" write error ; write_flag=%d!!!!! \n",write_flag);
		    }
		    fsync(fd);
		    close(fd);
		}			
		...
		if (memcmp(buf, verify, filesize) != 0) {
			LOCAL_LOG_ERR(" verification compare error");
		}
	...
	sync();
//	for(j=0; j < 0x10000 ; j++);		//sync会自己挂起直到sync过程结束。所以这里无需延时。
	for(j=i+1; j< FileNum  ; j++)				//每次删除一个文件，就做一下sync。防止一次sync时间过长。
	{
		system("rm /data/test/%d.txt" , j);
		sync();
	}
    system("rm /data/test/*.txt");
    sync();
